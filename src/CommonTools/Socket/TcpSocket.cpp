...
int CTcpSocket::TcpRead(void *const pvRecvBuf, size_t sizeRecvLen)
{
	...
	if(::setsockopt(m_iSockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&stTimeout, sizeof(stTimeout)) == -1) 
	{//设置读取超时 失败
		return -1;
	}
	...
	//printf("Line:%d,res=%d\n",__LINE__,res);
	//printf("Line:%d,pvRecvBuf=%s\n",__LINE__,(char*)pvRecvBuf);
...
int CTcpSocket::TcpRead2(void *const pvRecvBuf, size_t sizeRecvLen)
{
	if(pvRecvBuf == NULL || sizeRecvLen <= 0 || m_iSockfd == -1) {//参数有误
		return -1;
	}
...
int CTcpSocket::EncodingBase64(char * pInput, char * pOutput)
{
 ...
 for (i=0; i < loop; i++)
 {
 //chenyg change char to unsigned char at 20160601
  unsigned char a1 = (pInput[i*3] >> 2);
  unsigned char a2 = ( ((pInput[i*3] & 0x03) << 4) | (pInput[i*3+1] >> 4) );
  unsigned char a3 = ( ((pInput[i*3+1] & 0x0F) << 2) | ((pInput[i*3+2] & 0xC0) >> 6) );
  unsigned char a4 = (pInput[i*3+2] & 0x3F);
 ...
 if (remain == 1)
 {
  ...
  i = iSrcLen-1;
   //chenyg change char to unsigned char at 20160601
  unsigned char a1 = (pInput[i] >> 2);
  unsigned char a2 = ((pInput[i] & 0x03) << 4);
 ...
 else if (remain == 2)
 {
  ...
  i = iSrcLen-2;
   //chenyg change char to unsigned char at 20160601
  unsigned char a1 = (pInput[i] >> 2);
  unsigned char a2 = ( ((pInput[i] & 0x03) << 4) | (pInput[i+1] >> 4));
  unsigned char a3 = ( (pInput[i+1] & 0x0F) << 2);
...
int CTcpSocket::TcpSslReadLen( void* buf, int nBytes)
{
        ...
        //do {
				...
				//printf("Line:%d,SSL_ERROR_WANT_WRITE= %d\n",__LINE__,SSL_ERROR_WANT_WRITE);				
